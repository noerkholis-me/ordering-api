...
public class PickUpPointController extends BaseController {
    ...
    public static Result listPickupPoint(String filter, String sort, int offset, int limit, Long storeId) {
        Merchant ownMerchant = checkMerchantAccessAuthorization();
        if (ownMerchant != null) {
            Query<PickUpPointMerchant> query = PickUpPointRepository.find.where().eq("t0.is_deleted", false).eq("merchant", ownMerchant).eq("store_id", storeId != null && storeId != 0 ? storeId : 0).order("t0.id");
            try {

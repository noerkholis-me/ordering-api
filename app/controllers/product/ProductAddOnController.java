...
public class ProductAddOnController extends BaseController {
        ...
        if (ownMerchant != null) {
            ...
            if (productAddOn != null) {
                try {
                    ...
                    Query<ProductAddOn> queryProductAddOn = ProductAddOnRepository.find.where().eq("t0.product_id", productAddOn.getProductMerchant().id).eq("t0.is_active", true).eq("t0.is_deleted", false).eq("merchant", ownMerchant).order("t0.id");
    ...
    private static String validateRequest(ProductAddOnRequest productAddOnRequest) {
        ...
        if (productAddOnRequest.getProductType() == null || productAddOnRequest.getProductType() == "")
        ...
        if (ownMerchant != null) {
            ...
            try {
                ...
                for(ProductMerchantDetail productMerchantDetail : productMerchantDetails){
                    ...
                    Query<ProductAddOn> queryProductAddOn = ProductAddOnRepository.find.where().eq("t0.product_id", productMerchantDetail.getProductMerchant().id).eq("t0.is_deleted", false).eq("merchant", ownMerchant).order("t0.id");
                    List<ProductAddOn> dataAddOn = ProductAddOnRepository.getDataForAddOn(queryProductAddOn);
                    List<ProductMerchantAssignResponse.ProductAddOn> responsesProductAddOn = new ArrayList<>();

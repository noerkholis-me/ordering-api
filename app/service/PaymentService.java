...
public class PaymentService {
    ...
    private static final String qrCodeStaticPath = Play.application().configuration().getString("sandbox.transaction.create.qrcode.static.path");
    ...
    public ServiceResponse qrCodeStaticPayment(QrCodeStaticPaymentRequest qrCodeStaticPaymentRequest) {
        ServiceResponse serviceResponse = new ServiceResponse();
        try {
            OkHttpClient httpClient = new OkHttpClient();
            String url = sandboxUrl + qrCodeStaticPath;
            logger.info("Initiate Payment URL: " + url);
            String requestBody = objectMapper.writeValueAsString(qrCodeStaticPaymentRequest);
            logger.info("Initiate Payment Request: " + requestBody);
            RequestBody body = RequestBody.create(requestBody, MediaType.get("application/json; charset=utf-8"));
            Request request = new Request.Builder()
                    .url(url)

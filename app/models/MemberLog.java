...
public class MemberLog extends BaseModel {
    ...
    private static String generateToken(String username, String password) throws NoSuchAlgorithmException {
        return Encryption.SHA1(new Date().toString() + "HOKEBATOKEN" + username + password);
    }

    public static MemberLog loginMember(String deviceModel, String deviceType, String deviceId, Member member) {
        ...
        try {
            ...
            if(member.email!=null){
                ...
                passCode = "GOOGLE";
            } else if(member.appleUserId!=null){
                userCode = member.appleUserId;
                passCode = "APPLE";
            } else {
                return null;
            }
            ...
            log.deviceModel = deviceModel.toUpperCase();
            log.deviceType = deviceType.toUpperCase();
    ...
    public static boolean logoutMember(String token) {
        MemberLog log = MemberLog.find.where().eq("token", token).eq("is_active", true).eq("member_type", "member")
    ...
    public static MemberLog isMemberAuthorized(String token, String apiKey) {
        // validate token
        MemberLog log = MemberLog.find.where().eq("token", token).eq("is_active", true).eq("member_type", "member")
                .setMaxRows(1).findUnique();
    ...
    public static MemberLog getByToken(String token) {
        return MemberLog.find.where().eq("token", token).eq("is_active", true).setMaxRows(1).findUnique();
    }

    public static List<MemberLog> getListMemberLog(Member member) {

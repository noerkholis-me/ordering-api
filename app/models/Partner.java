...
//        model.odooId = odooId;
        ...
        Photo.saveRecord("brd",url, "", "", "", url, user.id, "admin", "Partner", model.id);
    }

	public static String validation(Partner model) {
		Partner uniqueCheck = Partner.find.where().eq("slug", model.slug).setMaxRows(1).findUnique();
		if (model.name.equals("")) {
			return "Name must not empty.";
		}
		if (uniqueCheck != null && model.id == null) {
			return "Partner with similar name already exist";
		}
		if ((model.imageUrl != null) && ((model.imageName == null || model.imageName.equals(""))
			...
			return "Please describe all information for partner's";
		}
		return null;
	}
	...
	public static Page<Partner> page(int page, int pageSize, String sortBy, String order, String name, Integer filter) {
		ExpressionList<Partner> qry = Partner.find
				.where()
				.ilike("name", "%" + name + "%")
				.eq("is_deleted", false);
	...
	public static List<Partner> getHomePage() {
		return Partner.find.where()
				.eq("is_deleted", false)
				.eq("status", true)
				.setMaxRows(10).orderBy("view_count desc").findList();
	}
